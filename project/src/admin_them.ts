import { urlserver, TthuongHieu, ISanPham, NguoiDung } from "./common.js";
//danh sách người dùng ở dashboard
...
export const lay_san_pham_admin = async (sosp: number = 9) => {
    try {
...
export const danh_sach_nguoi_dung = async () => {
    try {
        ...
        // Lưu trữ dữ liệu người dùng và trang hiện tại
        let users = data;
        let currentPage = 1;
        ...
        const displayPagination = (totalUsers) => {
            ...
            for (let i = 1; i <= totalPages; i++) {
                ...
                pageButton.textContent = String(i);  // hoặc pageButton.textContent = i.toString();
        ...
        const updateTableDisplay = (users) => {
            ...
            displayPagination(users.length); // Hiển thị phân trang
        };

        // Hiển thị người dùng ban đầu
        updateTableDisplay(users);
        ...
        document.getElementById('searchUser').addEventListener('input', (e) => {
            ...
            const filteredUsers = users.filter(user => {
                ...
                const phone = user.phone ? String(user.phone) : ''; // Convert phone to string

                return name.includes(searchQuery) || email.includes(searchQuery) || phone.includes(searchQuery);
            });

            // Cập nhật lại bảng và phân trang cho kết quả tìm kiếm
...
export const them_nguoi_dung = async (data: NguoiDung): Promise<NguoiDung> => {
    try {
        ...
        const response = await fetch(urlserver + '/nguoi_dung', {
            ...
            body: JSON.stringify(data), // Gửi dữ liệu người dùng dưới dạng JSON
        });
        ...
        // Giả sử server trả về người dùng vừa được thêm
        const result: NguoiDung = await response.json();
...
export const danh_sach_thuong_hieu = async () => {
    try {
        ...
        const data = await response.json();
        data.sort((a, b) => parseInt(b.id) - parseInt(a.id));
        let arr: TthuongHieu[] = data as TthuongHieu[]
        console.log(data);
        ...
        const updateTableDisplay = (brand) => {
            ...
            displayPagination(brand.length); // Hiển thị phân trang
        };
...
export const them_thuong_hieu = async (data: TthuongHieu): Promise<TthuongHieu> => {
    try {
        // Gửi yêu cầu POST để thêm người dùng
        const response = await fetch(urlserver + '/thuong_hieu', {
            method: 'POST',
        ...
        // Giả sử server trả về người dùng vừa được thêm
        const result: TthuongHieu = await response.json();
...
export const form_sua_thuong_hieu = async (id) => {
    ...
    document.getElementById('sua')?.addEventListener('click', async (event) => {
        ...
        // Ép kiểu các input để truy cập `.value`
        const id = (document.getElementById('id') as HTMLInputElement).value;
        const ten = (document.getElementById('name') as HTMLInputElement).value;
        const quocgia = (document.getElementById('quocgia') as HTMLInputElement).value;
...
//sửa brad
export const sua_thuong_hieu = async <T extends {
    id: number;
    ten: string;
    quocgia: string;
...
//xóa brand
export const xoa_thuong_hieu = async (btn: HTMLButtonElement) => {
    let id: string | null = btn.getAttribute('data-id');
    // Lấy id của thương hiệu cần xóa
    let hot: boolean = window.confirm('Muốn xóa à chắc chưa?');  // Hiển thị hộp thoại xác nhận
    if (!hot) {
        return;  // Nếu không xác nhận, thoát khỏi hàm
    }

    // Gửi yêu cầu DELETE tới server để xóa thương hiệu
    let otp = { method: 'DELETE' };

    let kq = await fetch(urlserver + `/thuong_hieu/${id}`, otp)
        .then(res => res.json())  // Chuyển phản hồi từ server thành đối tượng JSON
        .then(data => data);

    // Sau khi xóa, chuyển hướng về trang quản lý thương hiệu
...
export const danh_sach_san_pham = async () => {
    try {
        ...
        const updateTableDisplay = (products) => {
            ...
            displayPagination(products.length); // Hiển thị phân trang
        };
...
export const them_san_pham = async (data: ISanPham): Promise<ISanPham> => {
    try {
        const response = await fetch(urlserver + '/san_pham', {
            method: 'POST',
            headers: {
...
export const lay_thuong_hieu = async () => {
    try {
        ...
        const data = await response.json(); // Đặt await ở đây
        let kq = '';
        data.forEach(item => {
            kq += `<option value='${item.id}'>${item.ten}</option>`;
        });
        document.getElementById('thuonghieu').innerHTML = kq; // Sửa '=' thay vì '+=' để tránh lặp danh sách cũ
...
export const form_sua_san_pham = async (id) => {
    ...
    try {
        ...
        document.getElementById('editProductForm')?.addEventListener('submit', async (event) => {
            ...
            const ten = (document.getElementById('name') as HTMLInputElement).value;
            const gia = Number((document.getElementById('old-price') as HTMLInputElement).value);
            const gia_km = Number((document.getElementById('new-price') as HTMLInputElement).value);
            const hinh = (document.getElementById('image') as HTMLInputElement).value;
            const soluongtonkho = Number((document.getElementById('stock') as HTMLInputElement).value);
            const hot = (document.getElementById('hot') as HTMLSelectElement).value === "true";
            const an_hien = (document.getElementById('an_hien') as HTMLSelectElement).value === "true";
            const thuonghieu_id = Number((document.getElementById('thuonghieu') as HTMLSelectElement).value);
            const thumbnails = (document.getElementById('thumbnails') as HTMLInputElement).value.split(',').map(link => link.trim());
            const mota = (document.getElementById('mota') as HTMLTextAreaElement).value;
            const trangthai = (document.getElementById('status') as HTMLSelectElement).value;
            const tinhchat = Number((document.getElementById('tinhchat') as HTMLSelectElement).value);
