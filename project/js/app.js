...
export const them_gio_hang = async (sanPham) => {
    try {
        const response = await fetch(`${urlserver}/gio_hang`);
        const gioHangHienTai = await response.json();
        const soLuong = parseInt(document.querySelector('.shop-quantity input[name="quantity"]').value) || 1;
        ...
        const sanPhamTonTai = gioHangHienTai.find((item) => item.id === sanPham.id && item.userID === userID);
        if (sanPhamTonTai) {
...
export const them_gio_hang_home = async (id) => {
    try {
...
export const xoa_gio_hang = async (productId) => {
    try {
...
export const xoa_gio_hang_menu = async (productId) => {
    try {
        const response = await fetch(`${urlserver}/gio_hang/${productId}`, {
            method: 'DELETE',
            headers: {
...
export const dang_nhap = async (data) => {
    try {
        ...
        if (user.role === 'admin') {
            window.location.href = "/admin/adminthuancss.html";
        }
        else {
...
export const dang_nhap_menu = async (data) => {
    try {
        const response = await fetch(`${urlserver}/nguoi_dung`);
        if (!response.ok) {
            throw new Error('Không thể lấy dữ liệu người dùng');
...
export const dang_ky = async (data) => {
    try {
        const response = await fetch(`${urlserver}/nguoi_dung`, {
            method: 'POST',
            headers: {
...
export const dang_xuat = async () => {
    if (confirm('Bạn có chắc chắn muốn đăng xuất?')) {
        ...
        const showan = document.querySelector('.showan');
...
export const doi_mat_khau = async (oldPassword, newPassword) => {
    try {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (nguoiDung && Object.keys(nguoiDung).length > 0) {
        ...
        const accSidebarHeading = document.querySelector('.acc-sidebar h3');
        if (accSidebarHeading) {
            accSidebarHeading.innerHTML = `Xin chào, <b>${nguoiDung.name}</b>`;
        }
        const accDataName = document.querySelector('.acc-data[data-show="1"] p:nth-of-type(1)');
        if (accDataName) {
            accDataName.innerHTML = `<strong>Họ tên:</strong> ${nguoiDung.name}`;
        }
        const accDataEmail = document.querySelector('.acc-data[data-show="1"] p:nth-of-type(2)');
        if (accDataEmail) {
            accDataEmail.innerHTML = `<strong>Email:</strong> ${nguoiDung.email}`;
        }
        const accDataPhone = document.querySelector('.acc-data[data-show="1"] p:nth-of-type(3)');
        if (accDataPhone) {
            accDataPhone.innerHTML = `<strong>Điện thoại:</strong> ${nguoiDung.phone}`;
        }
        const accDataAddress = document.querySelector('.acc-data[data-show="1"] p:nth-of-type(4)');
...
lay_thuong_hieu();
export const laysptheothuonghieu = async (thuonghieu_id, sosp = 19) => {
    let str = ``;
    try {
...
export const lay_gio_hang_menu = async () => {
    try {
        const response = await fetch(`${urlserver}/gio_hang`);
        const data = await response.json();
        const showan = document.querySelector('.showanmenu');
        ...
        if (data.length === 0) {
            ...
            const productCount = data.length;
            if (cartHeader) {
                cartHeader.textContent = productCount.toString();
            }
            let str = '';
...
export const lay_san_pham = async (sosp = 3) => {
    try {
...
const tatcasp = (allsp) => {
    const { id, ten, gia, gia_km, hinh, soluongtonkho, hot, an_hien, mota, trangthai, tinh_chat, thumbnails, thuonghieu_id, } = allsp;
    let obj;
...
const renderProductDetail = (product) => {
    const { id, ten, gia, gia_km, hinh, soluongtonkho, hot, an_hien, mota, trangthai, tinh_chat, thumbnails, thuonghieu_id, } = product;
    let obj;
    obj = new CSan_Pham(id, ten, gia, gia_km, hinh, soluongtonkho, hot, an_hien, mota, trangthai, tinh_chat, thumbnails, thuonghieu_id);
...
const motsp = (sp) => {
    const { id, ten, gia, gia_km, hinh, soluongtonkho, hot, an_hien, mota, trangthai, tinh_chat, thumbnails, thuonghieu_id, } = sp;
    let obj;
    obj = new CSan_Pham(id, ten, gia, gia_km, hinh, soluongtonkho, hot, an_hien, mota, trangthai, tinh_chat, thumbnails, thuonghieu_id);
    return `
...
const gohangmenu = (giohangmenu) => {
    const { id, ten, gia, gia_km = 0, hinh = "default.jpg", thuonghieu_id = "Không rõ", size, soluong = 1 } = giohangmenu;
    let obj = {
        id,
        ten,
        gia,
...
const tatcagiohang = (allgiohang) => {
    const { id, ten, gia, gia_km = 0, hinh = "default.jpg", thuonghieu_id = "Không rõ", size, soluong = 1 } = allgiohang;
    let obj = {
        id,
        ten,
        gia,
